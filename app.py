...
DATA_PATH = BASE_DIR / "data" / "mock_child_support_cases.csv"
ZIP_PATH = BASE_DIR / "data" / "zip_reference.csv"
OVERRIDE_PATH = BASE_DIR / "county_overrides_template.json"
...
st.title("Child Support Fraud & Integrity Scorecard")
st.caption("Distance-adjusted framework for CP risk, NCP risk, and structural/system integrity in child support cases.")
...
with st.sidebar:
    ...
    selected_county = st.selectbox("County threshold profile", ["Default"] + sorted(load_data(uploaded_file)["county"].unique().tolist()))
    thresholds = overrides["default_thresholds"].copy()
    if selected_county != "Default":
        thresholds.update(overrides["county_overrides"].get(selected_county, {}))

    counties = sorted(load_data(uploaded_file)["county"].unique().tolist())
    regions = sorted(load_data(uploaded_file)["region"].unique().tolist())
